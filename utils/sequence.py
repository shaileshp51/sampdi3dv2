...
def get_protein_polymer_sequence(pdb_file, outfileprefix, derive_seq_from_coords=False):
    ...
    resmap_list = []
    fasta_file = f"{outfileprefix}_chainX.fasta"
    ...
    for chain in bb_atoms.getHierView().iterChains():
        ...
        # A. Process residues and their backbone atoms (coordinate-based)
        for residue in residues:
            residue_backbone = residue.select("name N CA C O")
            if residue_backbone is not None:
                for atom in residue_backbone:
                    ...
                    structural_residues.setdefault(reskey, []).append(atom.getName())

        # B. Add missing residues (REMARK 465)
        remark_residues = []
        if header and "missing" in header and chain_id in header["missing"]:
            remark_residues = header["missing"][chain_id]

        # C. Combine coordinate and missing residues
        ...
        if not is_header_missing and not derive_seq_from_coords:
            ...
            if len(seqres_declared) != len(coord_seq):
                logger_obj.warning(
                    f"[Chain {chain_id}] SEQRES length ({len(seqres_declared)}) "
                    f"≠ coordinate+missing length ({len(coord_seq)}). "
                ...
                for i, (declared, observed) in enumerate(zip(seqres_declared, coord_seq)):
                    if declared != observed:
                        logger_obj.debug(
                            ...
                            f"SEQRES={declared}, COORD={observed}"
                        )

            # Apply SEQADV-based filtering for expression tags
            ...
            final_residue_tuples = [
                r for r in initial_combined_residues if r[0] in final_pdb_resids
            ]

        else:
            # --- HEADER MISSING OR COORDINATES FORCED ---
            if logger_obj:
                logger_obj.debug(
                    "SEQRES/Header missing or ignored. Using coordinate-derived sequence as ground truth."
                )
            final_residue_tuples = initial_combined_residues

        # Sort and detect sequence gaps
        ...
        # Generate sequence
        sequence_one_letter = [
            three_to_one(res_name3) for _, _, res_name3 in final_residue_tuples
        ]
        ...
        fasta_file = f"{outfileprefix}_chain{chain_id}.fasta"
        with open(fasta_file, "w") as seq_file:
            seq_file.write(f">{outfileprefix}|chain {chain_id}|final-sequence\n")
            seq_file.write("".join(sequence_one_letter))
            seq_file.write("\n")
